...
# Controlador definitivo con OS-Ken + FlowManager
$WAN_EXEC osken-manager flowmanager/flowmanager.py os_ken.app.ofctl.service 2>&1 | tee osken.log &
...
echo "Esperando a que brwan se conecte al controlador..."
sleep 15

## 5. Aplica las reglas de la sdwan con ryu
#echo "## 5. Aplica las reglas de la sdwan con ryu"
echo "## 5. Aplica las reglas de la SD-WAN mediante la API REST del controlador"
RYU_ADD_URL="http://localhost:$PORTWAN/flowform"
curl -X POST -H "Content-Type: application/json" -d @json/from-cpe.json $RYU_ADD_URL
curl -X POST -H "Content-Type: application/json" -d @json/to-cpe.json $RYU_ADD_URL
curl -X POST -H "Content-Type: application/json" -d @json/broadcast-from-axs.json $RYU_ADD_URL
curl -X POST -H "Content-Type: application/json" -d @json/from-mpls.json $RYU_ADD_URL
curl -X POST -H "Content-Type: application/json" -d @json/to-voip-gw.json $RYU_ADD_URL
curl -X POST -H "Content-Type: application/json" -d @json/sdedge$NETNUM/to-voip.json $RYU_ADD_URL
